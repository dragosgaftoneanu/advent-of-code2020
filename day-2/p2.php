...
{
	$values = explode(" ", $password);
	
	for($i=0;$i<strlen($values[2]);$i++)
	{
		if($values[2][explode("-", $values[0])[0]-1] == substr($values[1], 0, 1) xor $values[2][explode("-", $values[0])[1]-1] == substr($values[1], 0, 1))
		{
			$valid++;
			break;
		}
