...
{
	preg_match('/(.*)byr:([0-9]{4})[\S+\n\s]+/', $passport, $matches);
	if(!$matches)
		continue;
	if($matches[2] < 1920 || $matches[2] > 2002)
		continue;
		
	
	preg_match('/(.*)iyr:([0-9]{4})[\S+\n\s]+/', $passport, $matches);
	if(!$matches)
		continue;
	if($matches[2] < 2010 || $matches[2] > 2020)
		continue;
	
	preg_match('/(.*)eyr:([0-9]{4})[\S+\n\s]+/', $passport, $matches);
	if(!$matches)
		continue;
	if($matches[2] < 2020 || $matches[2] > 2030)
		continue;
	
	preg_match('/(.*)hgt:(([0-9]{2,3})(cm|in))[\S+\n\s]+/', $passport, $matches);
	if(!$matches)
		continue;
	if($matches[4] == "cm" && ($matches[3] < 150 || $matches[3] > 193))
		continue;
	if($matches[4] == "in" && ($matches[3] < 59 || $matches[3] > 76))
		continue;
	
	preg_match('/(.*)hcl:#[a-f0-9]{6}[\S+\n\s]+/', $passport, $matches);
	if(!$matches)
		continue;
	
	preg_match('/(.*)ecl:(amb|blu|brn|gry|grn|hzl|oth)[\S+\n\s]+/', $passport, $matches);
	if(!$matches)
		continue;

	preg_match('/(.*)pid:[0-9]{9}[\S+\n\s]+/', $passport, $matches);
	if(!$matches)
		continue;
