...
while(true)
{
	...
	for($i=0;$i<count($rowsx);$i++)
	{
		...
		{	
			...
			{				
				if(checkAdjacentSeats($rows, $i, $j) >= 5)
				{
					$changed = 1;
					$rowsx[$i][$j] = "L";
				}
...
function checkAdjacentSeats($rows, $i, $j)
{
	...
	for($i1=$i-1;$i1>=0;$i1--)
	{
		...
		if($j - $diff >= 0)
		{
			if(in_array($rows[$i1][$j-$diff], array("#", "L")))
			{
				if($rows[$i1][$j-$diff] == "#")
					$sum++;
				break;
	...
	/* right */
	for($j1=$j+1; $j1<strlen($rows[$i]); $j1++)
	{
		if(in_array($rows[$i][$j1], array("#", "L")))
		{
			if($rows[$i][$j1] == "#")
				$sum++;
			break;
