...
while(true)
{
	...
	{
		for($j=0;$j<strlen($rowsx[$i]);$j++)
		{

			if($rows[$i][$j] == "L")
			{				
				if(checkAdjacentSeats($rows, $i, $j) == 0)
				{
					$rowsx[$i][$j] = "#";
					$changed = 1;
				}
			...
			{				
				if(checkAdjacentSeats($rows, $i, $j) >= 4)
				{
					$changed = 1;
					$rowsx[$i][$j] = "L";
				}
...
echo "Occupied seats: " . substr_count(implode("", $rows), "#") . "\n";

function checkAdjacentSeats($rows, $i, $j)
{
	$sum = 0;
	
	if($j > 0 && $rows[$i][$j-1] == "#")
		$sum++;
	if($j < strlen($rows[$i])-1 && $rows[$i][$j+1] == "#")
		$sum++;
	if($i > 0 && $j > 0 && $rows[$i-1][$j-1] == "#")
		$sum++;
	if($i < count($rows)-1 && $j > 0 && $rows[$i+1][$j-1] == "#")
		$sum++;
	if($i < count($rows)-1 && $j < strlen($rows[$i])-1 && $rows[$i+1][$j+1] == "#")
		$sum++;
	if($i < count($rows)-1 && $rows[$i+1][$j] == "#")
		$sum++;
	if($i > 0 && $rows[$i-1][$j] == "#")
		$sum++;
	if($i > 0 && $j < strlen($rows[$i])-1 && $rows[$i-1][$j+1] == "#")
		$sum++;
